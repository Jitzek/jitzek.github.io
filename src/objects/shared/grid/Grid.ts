...
export class Grid {
  ...
  public rearrangeGrid(
    screenWidth: number = this.prevScreenWidth,
    screenHeight: number = this.prevScreenHeight
  ) {
    ...
    this.gridItems.forEach((gridItem) => {
      ...
      if (
        gridItem.preferredColumn < 0 ||
        gridItem.preferredColumn > columnsPerRow
      ) {
        preferredColumn = columnsPerRow;
      }

      let emptyGridPositions = this.gridPositions.filter(
        (position) => position.item == null
      );
      if (emptyGridPositions.length > 0) {
        let preferredGridPosition = emptyGridPositions.reduce(
          (prev, current) => {
            return Math.abs(current.row - preferredRow) <
              Math.abs(prev.row - preferredRow) ||
              Math.abs(current.column - preferredColumn) <
                Math.abs(prev.column - preferredColumn)
              ? current
              : prev;
          }
        );
    ...
    return this.gridPositions.filter(filter).reduce((prev, current) => {
      return Math.abs(current.x - x) < Math.abs(prev.x - x) ||
        Math.abs(current.y - y) <
          Math.abs(prev.y - y) - convertRemToPixels(this.columnHeight)
        ? current
        : prev;
    });
  }
