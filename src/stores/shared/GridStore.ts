...
export function setGridParameters(
  gap: number,
  widthOffset: number,
  topOffset: number,
  bottomOffset: number,
...
export function selectGridItem(gridItem: GridItem) {
  _gridStore.update((_grid) => {
    _grid.gridItems.find(
      (_gridItem) => _gridItem.id === gridItem.id
    ).selected = true;
    return _grid;
  });
}
...
export function setPreferredPositionOfGridItem(
  gridItem: GridItem,
  preferredRow: number,
  preferredColumn: number
) {
  _gridStore.update((_grid) => {
    let _updatedGridItem = _grid.gridItems.find(
      (_gridItem) => _gridItem.id === gridItem.id
    );
